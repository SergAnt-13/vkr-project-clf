...
class BERTOKPDClassifier:
    ...
    def prepare_data(self, df: pd.DataFrame, text_column: str = "name_norm", 
                    label_column: str = "okpd2_current", min_samples_per_class: int = 10) -> Tuple[List[str], List[int]]:
        ...
        logger.info(f"После фильтрации редких классов: {len(filtered_df)} записей, {len(frequent_codes)} уникальных кодов")
    ...
    def fit(self, df: pd.DataFrame, text_column: str = "name_norm", 
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                if batch_idx % 50 == 0:
                    logger.info(f"Эпоха {epoch+1}/{num_epochs}, Батч {batch_idx}/{len(train_loader)}, Потери: {loss.item():.4f}")
    ...
    def predict(self, texts: List[str], batch_size: int = 32) -> Tuple[List[str], List[float]]:
        ...
        dataset = OKPDDataset(texts, [0] * len(texts), self.tokenizer)  # Фиктивные лейблы
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                for pred_id, conf in zip(predicted_ids.cpu().numpy(), max_probs.cpu().numpy()):
